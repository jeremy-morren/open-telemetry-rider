...
val verifyPlatformVersions = providers.gradleProperty("verifyPlatformVersions")
    .orElse(platformVersion)
    .map { raw ->
        raw.split(',')
            .map { it.trim() }
...
intellijPlatform {
    ...
    signing {
        certificateChain = file(providers.gradleProperty("signingCertificate")).readText(Charsets.UTF_8)
        privateKey = providers.environmentVariable("SIGNING_KEY")
        password = providers.environmentVariable("SIGNING_KEY_PASSWORD")
    }
    ...
    pluginVerification {
        ides {
            verifyPlatformVersions.get().forEach { verifyVersion ->
                create(
